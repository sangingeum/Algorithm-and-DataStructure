...
float ZeroOneKnapsack::solve(const std::vector<float>& values, const std::vector<float>& weights, size_t capacity) {
...
std::pair<float, std::vector<size_t>> ZeroOneKnapsack::solveAndTrace(const std::vector<float>& values, const std::vector<float>& weights, size_t capacity) {
	size_t n = values.size();
	std::vector<std::vector<float>> dp(n + 1, std::vector<float>(capacity + 1, 0));
