...
public:
	...
	AdjacencyMatrixGraph<N, D>(size_t numVertices, D initalValue);
...
AdjacencyMatrixGraph<N, D>::AdjacencyMatrixGraph(size_t numVertices) : Graph<N, D>(numVertices) {
...
// Create (numVertices x numVertices) AdjacencyMatrices with the given inital value for edges
template<class N, class D>
inline AdjacencyMatrixGraph<N, D>::AdjacencyMatrixGraph(size_t numVertices, D initalValue) : Graph<N, D>(numVertices)
{
	for (auto& edges : this->m_edges) {
		edges = std::move(std::vector<size_t>(numVertices, 0));
	}
	for (auto& atts : this->m_edgeAttributes) {
		atts = std::move(std::vector<D>(numVertices, initalValue));
	}
}
